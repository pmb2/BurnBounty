...
- Start: `/`
- Target: authenticated user reaches `/commit`
...
#### After
- Home has one dominant CTA: `Start Playing` (`/auth?mode=embedded&next=/commit`).
...
2. Auth: fill username/passphrase, click `Create Embedded Account`
3. Auto-redirect to `/commit`
...
1. Add explicit progress stepper (`Access -> Commit -> Reveal -> Collection`) in shared layout.
...
4. Improve empty states in protected pages (`/collection`, `/trading`, `/dashboard`) for post-auth redirection clarity.
5. Add route-level UX smoke script (Playwright) once MCP browser transport is stable.
